...
/*  
  ...
  computeTHEBDs()
  {
   ...
   combinationsID_Type combinationsID( nbPz - 1, this->M_J, true );
   ...
   for ( UInt iPz(0); iPz < nbMyPz; ++iPz )
   {
	...
	std::vector< Real > currentValues( this->M_J );
	...
	this->M_BDs[ iPz ] = comprisedCounter / static_cast< Real > ( ( nbPts - 1 ) * binomial( nbPz - 1, this->M_J ) );
  }	    
  ...
  */
  ...
  BDRefDirectComputation::
  BDRefDirectComputation
  ( const bdRefData_Type & bdRefData )
  :
  BandDepthRef( bdRefData ),
  M_J(2)
  {
  }
  ...
  computeTHEBDs()
  {
   ...
   combinationsID_Type combinationsID( nbRefSamples, this->M_J, true );
   ...
   for ( UInt iSample(0); iSample < nbMySamples; ++iSample )
   {
	...
	this->M_BDs[ iSample ] = comprisedCounter / static_cast< Real > ( ( nbPts - 1 ) * binomial( nbRefSamples, this->M_J ) );
    }	    
